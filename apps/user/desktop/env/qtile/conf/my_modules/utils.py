...
def get_monitor_status() -> dict:
    ...
    status["connected"] = subprocess.run(
        connected, shell=True, capture_output=True, text=True
    ).stdout.split("\n")[:-1]
    disconnected = xrandr + r" | grep disconnected | awk -F' ' '{print $1}'"
    status["disconnected"] = subprocess.run(
        disconnected, shell=True, capture_output=True, text=True
    ).stdout.split("\n")[:-1]
